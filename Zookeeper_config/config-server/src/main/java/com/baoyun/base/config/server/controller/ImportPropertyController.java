...
public class ImportPropertyController {	
	...
	public NoPageResultModel importProperty(
		...
		log.info("txNo: {}, fileName: {}", txNo, file.getOriginalFilename());
		...
		try {
			...
			if ((boolean) restModel.getData()) 
				log.info("import property success, txNo: {} fileName", txNo,file.getName());
			 else 
				log.error("import property failed, txNo: {}, fileName: {}", txNo, file.getName());
			return restModel;
